...
module.exports = class User extends Model {
    ...
    async getFavoriteMovies() {
        const { FavoriteMovie } = this.constructor;
        return await FavoriteMovie.query().where({ userId: this.id }).withGraphFetched('movie');
    }

    async addFavoriteMovie(movieId) {
        const favoriteMovie = await FavoriteMovie.query().findOne({ userId: this.id, movieId });
        if (favoriteMovie) {
            throw new Error('Ce film est déjà dans vos favoris');
        }
        await FavoriteMovie.query().insert({ userId: this.id, movieId });
    }

    async removeFavoriteMovie(movieId) {
        const favoriteMovie = await FavoriteMovie.query().findOne({ userId: this.id, movieId });
        if (!favoriteMovie) {
            throw new Error('Ce film n\'est pas dans vos favoris');
        }
        await FavoriteMovie.query().delete().where({ userId: this.id, movieId });
    }
